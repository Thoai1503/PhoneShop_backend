...
let PaymentService = class PaymentService {
    ...
    async createVNPayPayment(payload, req) {
        ...
        const finalAmount = resolvedAmount > 0
            ? resolvedAmount
    ...
    async handleVNPayReturn(query) {
        ...
        const orderInfo = this.normalizeOrderInfo(String(query.vnp_OrderInfo || ''));
    ...
    async createMoMoPayment(payload, req) {
        const userId = payload.user_id;
        const amount = Number(payload.amount || 0);
        const orderInfoPayload = this.normalizeOrderInfo(payload.orderInfo);
        const addressId = payload.address_id ??
            (Number(orderInfoPayload.address_id ?? 0) || undefined);
        const finalAmount = amount > 0 ? amount : await this.estimateCartTotal(userId);
        const { orderId } = await this.repo.createOrderFromCart({
            userId,
            amount: finalAmount,
            addressId,
        ...
        if (!response.ok) {
            throw new Error(`MoMo payment request failed with status ${response.status}`);
        }
    ...
    async estimateCartTotal(userId) {
...
PaymentService = __decorate([
    Injectable(),
    __param(0, Inject(PaymentRepository)),
    __metadata("design:paramtypes", [PaymentRepository])
], PaymentService);
